...
exports.index = async(req, res, next) => {
    ...
    const data = require('./poem/poem.json');
    const path_D = path.join(__dirname, 'poem', 'poem.json');
    if (!fs.existsSync(path_D)) { fs.writeFileSync(path_D, '[]', 'utf-8'); }
    if (req.query.type == 'upload') {
        let link = req.query.word;
        ...
        fs.writeFileSync(path_D, JSON.stringify(link, null, 4), 'utf8');
        return res.json({
            success: 200,
            data: link
        })
