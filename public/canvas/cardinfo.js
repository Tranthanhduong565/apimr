...
const fontsLink = 40
    ...
    const { location, name, gender, vanity, uid } = req.query;
    if (!location || !name || !gender || !vanity || !uid) {
      return res.json({
        error: 'Thiếu dữ liệu để thực hiện chương trình tạo ảnh!',
        example: {
          message: "https://api-test.sp20bse.repl.co/cardinfo?location=Hưng Yên&name=Công Quyền&gender=nam&vanity=kaneki&uid=100040984926932"
        }
      })
    }
    else {
      ...
      if (!fs.existsSync( __dirname + `./cache/VT323-Regular.otf`)) {
        let getfont = (await axios.get(`https://github.com/tdunguwu/font/blob/main/VT323-Regular.ttf?raw=true`, {
          ...
          .data;
        fs.writeFileSync(__dirname + `/cache/VT323-Regular.otf`, Buffer.from(getfont, "utf-8"));
      };
